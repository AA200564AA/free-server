...
# Install essentials (python3 for full stdlib, including http.server)
RUN apt-get update && \
    DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends \
        openssh-server curl ca-certificates net-tools sudo python3 && \
...
# Startup script (fixed variable escaping + python3 full install)
